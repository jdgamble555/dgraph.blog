<script lang="ts">
	...
	import { confirmSignIn, user, loginWithGoogle, sendEmailLink } from '../modules/firebase';
	...
	// stores
	import { showDialog, showAuthSettings, showSnackbarMsg, userState } from '../stores/core';
	...
	onMount(() => {
		...
		userSub = user.subscribe(async (u: any) => {
			...
			if (u) {
				...
				// get user
				let { data }: any = await userService.getUser(u);
	...
	async function updateUser(user: any) {
		const { data }: any = await userService.updateUser($userState.id, user);
		userState.set(data);
...
<Dialog bind:active={$showDialog}>
	<Card>
		...
		<CardText>
			<center>
				<TextField bind:value={userRec.email} dense outlined>Enter your email address...</TextField>
				<br />
				{#if showEmail}
					<span
						on:click={() => {
				...
				{:else}
					...
					<br />
					<br />
					<Button class="red white-text" on:click={loginWithGoogle}>Signin with Google</Button>
				{/if}
			</center>
		</CardText>
	</Card>
...
<Dialog bind:active={$showAuthSettings}>
	<Card>
		...
		<CardText>
			<TextField bind:value={userRec.displayName} dense outlined>Display Name</TextField>
			<br />
			...
			<Button style="margin: 1em" on:click={() => showAuthSettings.set(false)}>Cancel</Button>
		</CardText>
	</Card>
</Dialog>
